 ...
 * Tested up to: 4.4.2
 * Text Domain: smart-reading-time-tdom
 * Domain Path: /languages/
 * @package Smart Reading Time
 * @author Sebastien Bosquet
 * -------------------------------------------------------
 * Copyright © 2004 - 2016 Sebastien Bosquet ALL RIGHTS RESERVED
 * -------------------------------------------------------
 */
...
function smart_reading_time_load_languages() {
	...
	$dummy_desc = __( 'Estimates an average required time to complete reading a post.', 'smart-reading-time-tdom' );
}
...
class SmartReadingTimeWP {
	...
	public function srt_add_reading_time_before_content($content) {
		...
		$timeaftersrt = $srtReadingOptions['aftersrt'];
		...
		$stiff.= '<div class="srt-reading-time">'.'<span class="srt-designation">'.$designation.'</span> '.'<span class="srt-time">'.$this->SmartReadingTime.'</span>'.' <span class="srt-after-time">'.$timeaftersrt.'</span>'.'</div>';
...
$SBUpdateChecker = PucFactory::buildUpdateChecker('http://www.sebastienbosquet.com/plugin-update-server/?action=get_metadata&slug=smart-reading-time',__FILE__,'smart-reading-time');
